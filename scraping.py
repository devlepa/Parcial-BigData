...
def scraping(s3, bucket_name):
    ...
    for archivo in archivos_en_s3:
        ...
        for listing in listings:
            contenido = listing.find(
                "div", class_="listing-card__information__main")
            if not contenido:
                continue  # Si no hay contenido, saltamos

            valor = contenido.find(
                "span", class_="price__actual").text.strip() if contenido.find(
                "span", class_="price__actual") else "N/A"
            barrio = contenido.find(
                "div", class_="listing-card__location__geo").text.strip() if contenido.find(
                "div", class_="listing-card__location__geo") else "N/A"

            # Buscar las propiedades
            propiedades = listing.find_all(
                "div", class_=lambda x: x and "listing-card__properties__property" in x)
            ...
            # Agrupar los textos en grupos de tres (o según la estructura que
            # necesites)
            datos_agrupados = [", ".join(momento[i:i + 3])
                               for i in range(0, len(momento), 3)]
